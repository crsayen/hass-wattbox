...
def parse_outlet_power_response(response: str) -> tuple:
    ...
    return (
        int(values[0]),      # outlet index
        float(values[1]),    # power watts
        float(values[2]),    # current amps
        float(values[3])     # voltage volts
    )
...
def parse_ups_status_response(response: str) -> UPSStatus:
    ...
    return UPSStatus(
        battery_charge_percent=int(values[0]),
        battery_load_percent=int(values[1]),
        battery_health=values[2],
        power_lost=values[3].lower() == "true",
        battery_runtime_minutes=int(values[4]),
